...
```bash
/plugin marketplace add YOUR_USERNAME/claude-usage-hud
/plugin install claude-usage-hud
...
```
🐙 Opus 4.6 │ 👨‍💻 73%(4h32m) │ 📅 Week: 45%(3d21h) │ 42% ctx │ $0.47
```

Sonnet 사용 시:

```
☄️ Sonnet 4.6 │ 👨‍💻 31%(3h12m) │ 📅 Week: 22%(5d8h) │ 18% ctx │ $0.12
```

업데이트가 필요한 경우:

```
🐙 Opus 4.6 │ 👨‍💻 73%(4h32m) │ 📅 Week: 45%(3d21h) │ 42% ctx │ $0.47 │ Update 👾
```
...
```
claude-usage-hud/                          # GitHub repo root
├── .claude-plugin/
│   └── plugin.json                        # Plugin manifest (필수)
├── commands/
│   ├── setup.md                           # /claude-usage-hud:setup
│   └── status.md                          # /claude-usage-hud:status
├── hooks/
...
```
~/.claude/plugins/data/claude-usage-hud/
...
```json
{
  ...
  "repository": {
    "type": "git",
    "url": "https://github.com/YOUR_USERNAME/claude-usage-hud"
  },
  "license": "MIT"
}
```
...
> `${CLAUDE_PLUGIN_DATA}`는 `~/.claude/plugins/data/claude-usage-hud/`로 해석된다. 플러그인 업데이트 시에도 데이터가 유지된다.

### 캐시 경로 결정 (usage-hud.mjs 내부)

```javascript
// CLAUDE_PLUGIN_DATA가 없으면 (수동 설치 등) fallback
const CACHE_DIR = process.env.CLAUDE_PLUGIN_DATA
...
```json
{
  ...
  "repository": "https://github.com/YOUR_USERNAME/claude-usage-hud",
  "license": "MIT",
...
### `/claude-usage-hud:setup` — `commands/setup.md`
...
### `/claude-usage-hud:status` — `commands/status.md`
...
```json
{
  ...
  "plugins": [
    {
      "name": "claude-usage-hud",
      "source": "./plugins/claude-usage-hud",
...
- [ ] `/claude-usage-hud:setup` 실행 시 진단 결과 출력
- [ ] `/claude-usage-hud:status` 실행 시 캐시 상태 + 토큰 상태 출력
